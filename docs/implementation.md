...
Lockout counters are shared by `REMOTE_ADDR` across projects; each project supplies its threshold. A system setting supplies the duration. Checks do not extend the deadline, and successful authentication clears that address's counter. Expired and malformed entries are pruned during locked mutations, and the shared JSON is capped at 10,000 addresses and 2 MiB. When system lockout time is positive, a per-IP MySQL advisory lock covers threshold checking, password verification and failure/success bookkeeping across sessions and projects. Acquisition waits at most five seconds; a timeout rejects the request before password verification. The lock is released even on backend or storage exceptions. A separate, short installation-wide lock covers only the stored JSON read/modify/write operation, so a slow LDAP bind does not serialize other IPs. Both locks and the protected storage reads use the primary connection. Framework setting helpers perform writes; direct core queries are used where explicit primary-connection control is required.
